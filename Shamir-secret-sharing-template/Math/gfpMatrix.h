...
void decompose_bits(gfpScalar x, const size_t &bit_length, Eigen::MatrixBase<Derived>&res)
{
    ...
    assert(res.rows() == 1);
    res.setConstant(0);  // 将结果矩阵所有元素初始化为0
    int idx = 0;
    while(!x.is_zero()){  // 当x不为0时继续循环
        res(idx) = x&1;   // 获取最低位（通过位与运算）
        idx++;            // 移动到下一位
        x >>= 1;         // 右移一位，准备处理下一个比特
    }
}
...
inline void batch_inversion(const gfpMatrix&a, gfpMatrix&a_inv)
{
    ...
    // 使用Eigen::seqN(1, a.size()-1)生成一个从1到a.size()-1的序列
    // 使用reshaped<Eigen::RowMajor>()将矩阵转换为行优先存储
    // 使用array()将矩阵转换为数组
    // 使用.array()将矩阵的每个元素转换为数组
    // 使用*运算符将两个数组相乘
    // 使用reshaped<Eigen::RowMajor>()将结果转换回矩阵
