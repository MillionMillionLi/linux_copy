...
using TYPE = unsigned int;//{unsigned int, uint64_t}, 无符号整数
using DTYPE = uint64_t;//{uint64_t, __uint128_t}, 无符号整数
using STYPE = int;//{int, int64_t}，有符号整数
using u64 = uint64_t;
// {31, 61, 127}
...
using TYPE = uint64_t;//{unsigned int, uint64_t}, 64bit无符号整数
using DTYPE = __uint128_t;//{uint64_t, __uint128_t}，128bit无符号整数
using STYPE = int64_t;//{int, int64_t}，64bit有符号整数
using u64 = uint64_t;
...
class gfpScalar
{
    ...
    friend gfpScalar sqrt(const gfpScalar &x);// vs gfpScalar& sqrt()：sqrt()returns a reference to the modified object；sqrt(const gfpScalar &x) returns a new gfpScalar object
...
protected:
    TYPE value; //true value in gfpScalar
public:
    gfpScalar()=default;
...
//NumTraits 是 Eigen 库用来定义数学类型特性的结构体
//<hmmpc::gfpScalar> 表示这些特性是专门为 gfpScalar 类型定义的
//: NumTraits<unsigned int> 表示这些特性是基于无符号整数的特性来定义的，但可以根据需要进行修改
{
