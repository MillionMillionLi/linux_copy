...
public:
    ...
    ShareBundle& reduce_truncate(size_t logLearningRate, size_t logMiniBatch);// 可以根据学习率和批次大小来调整截断的精度
    ShareBundle& reduce_truncate(size_t precision);//reduce [x]_2t -> [x/2^p]_t
    ShareBundle& reduce_truncate(vector<size_t> &precision);// reduce [x]_2t -> [x/2^{p1}, x/2^{p2}, ...]_t

    // Two layer multiplication: compute [xy] from input [x]_2t and [y] (input wire) 单个乘法
    ShareBundle& reduce_degree_1stLayer(const ShareBundle &y, BeaverTriple &triple);
    // Generalize two layer-mult: compute ([xy1], [xy2], ...) from input [x]_2t and ([y1], [y2], ...) 多个乘法
    ...
    gfpMatrix bound_power()const;// Bound power on each entry x to get alpha where 2^alpha <= x < 2^alpha+1, $\alpha = \lfloor \log_2 x \rfloor$ 
